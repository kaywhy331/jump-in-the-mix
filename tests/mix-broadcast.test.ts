...
describe("fixed-date broadcast schedules", () => {
  ...
  it("rejects normalized or impossible dates instead of silently shifting them", () => {
    expect(() => parseBroadcastScheduleInput("2026-02-30", "10:00", "UTC")).toThrow("valid broadcast date");
    expect(() => parseBroadcastScheduleInput("12/31/2026", "10:00", "UTC")).toThrow("valid broadcast date");
  });
  ...
  it("validates times and timezones", () => {
    ...
    expect(() => parseBroadcastScheduleInput("2026-12-31", "24:00", "UTC")).toThrow("valid broadcast time");
    expect(() => parseBroadcastScheduleInput("2026-12-31", "10:00", "Not/A_Timezone")).toThrow("valid broadcast timezone");
  });
