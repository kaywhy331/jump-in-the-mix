...
function preflight(triggerMode: "DATE_TRIGGERED" | "MANUAL_START" = "DATE_TRIGGERED") {
  return parseAiMixPreflight({
    objective: "Renewal preparation",
    tone: "Warm",
    framework: "Question-Led Consultative",
    triggerMode,
    dateTypeId: triggerMode === "DATE_TRIGGERED" ? "date-type" : null,
    dateTypeName: triggerMode === "DATE_TRIGGERED" ? "Renewal" : null,
    groupIds: ["group-one"],
    groupNames: ["Clients"],
...
const draft = {
  ...
  description: "A relationship-centered sequence before and after the renewal date.",
  category: "Client Follow-Up",
  industry: "Other",
  framework: "Question-Led Consultative",
  durationDays: 30,
  steps: [
    { name: "Prepare", channel: "EMAIL", dayOffset: -14, sendTimeMinutes: 600, subject: "A quick renewal question", body: "Hi {{First Name}}, what would be most useful before renewal?", script: null, longSms: false, includeOptOut: false },
    { name: "Call", channel: "PHONE_CALL", dayOffset: -7, sendTimeMinutes: 600, subject: null, body: null, script: "Ask what has changed and what the next renewal should accomplish.", longSms: false, includeOptOut: false }
  ]
};
...
describe("Mix authoring convergence", () => {
  ...
  it("allows inline actions and indexes every server-bound field", () => {
    ...
    expect(action).toContain('name: `inlineName-${index}`');
  });
