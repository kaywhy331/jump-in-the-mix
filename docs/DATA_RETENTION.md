...
Migration 47 indexes the contact referrer and follow-up mix, beat and version relationships. PostgreSQL does not automatically index the referencing side of a foreign key; deletion otherwise repeatedly scans remaining accounts' rows. A local PostgreSQL 16 profile of two synthetic accounts (1,000 contacts, 25,000 follow-ups and 30 mixes each) reduced workspace cascade execution from 2,367 ms to 509 ms. These are local database timings, not hosted request latency or a capacity promise. See [PostgreSQL foreign-key guidance](https://www.postgresql.org/docs/16/ddl-constraints.html#DDL-CONSTRAINTS-FK).

The same populated fixture exceeded Prisma's default five-second transaction deadline on the actual small hosted database, including with the new indexes alone. Account deletion therefore has a dedicated 30-second transaction limit. Its staff/admission locks, last-Owner protection, atomic erasure and completion audit remain in the same transaction. Other transactions keep their existing limits. A failure rolls back the deletion and never records it as completed.

The hosted-fixture regression exercises the real deletion function with a restricted database login, verifies complete customer-data removal and repeat deletion, preserves the other populated account, and requires source-account preservation and fixture cleanup. Enable it with `RUN_HOSTED_LOAD_WEB_TESTS=true` after preparing the standalone package. The indexes are additive; apply the reviewed migration with the isolated owner job before deploying the new web/worker release. The migration has a five-second lock-wait limit and leaves no partially installed indexes if it fails.

On September 9, the exact candidate source and migration SQL passed a Render one-off job against an isolated database on the existing PostgreSQL plan (0.1 CPU, 256 MB). The two existing account-deletion cases passed; deleting the representative account took 14,357 ms, all measured owned rows were removed, and the other fixture account remained. Source read/write attempts were denied and database/role/credential cleanup was verified. This qualifies the candidate function and cascades; release deployment and general hosted load qualification are separate checks. It does not establish deletion from backups, logs or mailboxes.
