...
**Status:** The production app and continuous worker are live at `https://jumpinthemix.com`. Route 53/TLS cutover, the verified sender, support forwarding, Owner access with MFA, the simplified admin workspace, restricted runtime database credentials and scheduled independent backups are established. Public launch still requires qualified performance and monitoring, remaining provider/device evidence, and the Owner’s admission limits and launch date. See [the completion ledger](PRODUCT_COMPLETION.md) for dated evidence.
...
The complete waitlist, wave-release, permissions, reporting, and operating design is in [Waitlist, referrals, and live administration](ADMIN_OPERATIONS_INFRASTRUCTURE_PLAN.md). The waitlist, weekly waves, manual invitations, named staff permissions, staff invitation onboarding, reports, retention, mix publication/rollback and email recovery are deployed. Admission ceilings remain zero and automatic waves have no launch schedule; deployment does not activate invitations. See [Waitlist operation](WAITLIST_OPERATIONS.md) for setup and operator steps.
...
| --- | --- | --- |
| Domain and DNS | Keep registration and authoritative DNS in AWS Route 53. | The Route 53 zone serves the new Render application. Base44 has been replaced; the canonical domain and TLS are verified. |
| Public website and app | One Next.js deployment on Render at `https://jumpinthemix.com`, including the homepage, registration, and signed-in app. Redirect `www.jumpinthemix.com` to the canonical domain. | Live at the canonical domain; `www` redirects to the apex. |
| Background processing | One continuously running Render worker, released from the same revision as the web app. Use the existing PostgreSQL job queue. | The continuous worker is live at the same application revision as the web app. Heartbeat, restricted database use and a hosted cleanup checkpoint passed; remaining contention and recovery exercises are tracked separately. |
| Production database | Separate managed Render PostgreSQL database, private to production services, with point-in-time recovery and a tested backup/restore procedure. | Private managed PostgreSQL 16 in Oregon, with 0.1 CPU, 256 MiB RAM and 5 GB storage. Migration 48 is applied. Render reports PITR available; independent encrypted backup and restore have passed. The actual workspace tier and Render-managed restore procedure remain to qualify. |
| Testing | Keep the password-protected `jump-in-the-mix-test.netlify.app` site and its Neon database separate from production. | The existing private test site is a separate environment; its current deployment was not requalified in this production checkpoint. Production releases have their own Render checks and isolated synthetic qualification. |
| Transactional email | Resend for account verification, password recovery, and enabled application email. | The production sender is verified and three actual account messages have provider-delivered receipts. Additional staff/support flows and mailbox placement still need qualification. |
...
| Support email | A monitored address such as `support@jumpinthemix.com`, connected to an actual mailbox or help desk. | ImprovMX forwards `support@jumpinthemix.com` and `hello@jumpinthemix.com` to the Owner’s designated Gmail inbox. Forwarding readiness is verified; actual inbox receipt and mailbox handling remain to confirm. |
| Operations | Error reporting, web/worker/database monitoring, alert delivery, independent backup storage, and usage alerts. | Web/worker health and daily encrypted S3 backups are active, with independent SSM key recovery and a held restore verified. The prepared continuous monitor and alert channel await the existing spending and outgoing-message approval. |

Render is the production destination; Route 53 remains authoritative DNS. The old Base44 site has been replaced. AWS also stores independent encrypted backups and separately protected recovery keys.
...
Current code contains the homepage waitlist, email confirmation, durable weekly schedule, atomic 5 FIFO + 5 random selection, manual selection, delivery outbox, referral exclusion, history, pause/resume, and waitlist audit records. These require explicit staff permissions and MFA. The staff console works without a customer workspace and supports individual overrides, immediate session revocation, and last-Owner protection; see [Staff access](STAFF_ACCESS.md). Staff invitation onboarding, admission limits and independent pause controls are deployed; see [Admission controls](ADMISSION_CONTROLS.md). Limits start at zero and require an explicit administrator configuration before new grants. Retention automation, suppression clearance, live/saved reports and support-email recovery are deployed; see [the completion ledger](PRODUCT_COMPLETION.md) for current evidence and remaining hosted qualification. Signed provider delivery events, permanent-bounce/complaint suppression, sending budgets with an account reserve, and email diagnostics are deployed; see [Email operations](EMAIL_OPERATIONS.md). Recipient withdrawal, confirmed rejoining, a shared member/platform invitation outbox, and an Invitations admin page are deployed. See the waitlist runbook for cancellation and safe retry boundaries.
...
Start with email delivery and password signup plus email verification. Google and Apple remain optional for existing-account sign-in. Native apps and payments are deferred. Production provisioning and DNS cutover are complete. Wave activation and additional staff invitations remain deliberate Owner actions after the relevant qualification.
...
Paid Postgres on Hobby includes a three-day point-in-time recovery window; independently test encrypted exports and restores. The canonical production cutover is verified.
...
The continuous worker is deployed on the intended production runtime. Continue observing it and complete the remaining recovery exercises. Verify due work and notifications while nobody browses the app, recovery after a worker restart, and prevention of duplicate actions. Alert when the heartbeat becomes stale or jobs repeatedly fail. The test site's recorded scheduler gaps exceeding 90 minutes remain unresolved evidence; activity-triggered processing does not close this gate.
