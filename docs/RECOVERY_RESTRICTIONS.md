...
The hold remains mandatory. There is no supported release command yet. This restriction operation does not reconstruct omitted notes, corrected contact details, newer accounts, retained content or newer accepted-send receipts. Per-table differences can therefore remain after apply. For complete contents, restore a newer [full recovery bundle](RECOVERY_BUNDLES.md) into a fresh target, then prepare a new restriction plan using its matching sidecar and manifest. The bundle path recovers recorded contents at capture; its available-source finalization command can supply a verified cutoff. It does not merge omitted changes into this older target.

The remaining recovery work includes coverage through the actual final source cutoff or independently retained changes; reconciliation of omitted corrections and delivery history; safe re-creation of necessary background work; fresh operator/customer access; obsolete integration-key handling; guarded release; and qualification of backup storage, keys, retention and the intended hosted runtime. No receipt from this command authorizes reopening or proves those requirements complete.
