...
export function parseBroadcastScheduleInput(dateValue: string, timeValue: string, timezoneValue: string): BroadcastScheduleInput {
  ...
  if (!DATE_INPUT.test(dateInput)) throw new Error("Choose a valid broadcast date.");
  ...
  if (Number.isNaN(localDate.getTime()) || localDate.toISOString().slice(0, 10) !== dateInput) {
    throw new Error("Choose a valid broadcast date.");
  }
  const timeMinutes = parseTimeInput(timeInput);
  if (timeMinutes === null) throw new Error("Choose a valid broadcast time.");
  if (!isValidTimezone(timezone)) throw new Error("Choose a valid broadcast timezone.");
