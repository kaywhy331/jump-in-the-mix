...
function accountIds(account: VerifiedGoogleAccount): string[] {
  return [account.sub?.trim(), account.email?.trim().toLowerCase()].filter((value): value is string => Boolean(value));
}
...
export async function exchangeGoogleAuthorizationCodeForVerifiedAccount(input: {
  ...
  const verifiedIds = accountIds(account);
  if (!verifiedIds.length) throw new Error("Google did not return a stable account identifier.");
  const existingIds = [input.existingExternalAccountId, input.existingAccountEmail]
    .map((value) => value?.trim().toLowerCase())
    .filter((value): value is string => Boolean(value));
  const sameKnownAccount = existingIds.length > 0 && existingIds.some((value) => verifiedIds.map((item) => item.toLowerCase()).includes(value));
  const accountChanged = existingIds.length > 0 && !sameKnownAccount;
  const refreshToken = token.refresh_token
    || (sameKnownAccount ? input.existingRefreshToken?.trim() : "")
    || "";
  if (!refreshToken) {
    throw new Error(
      accountChanged
