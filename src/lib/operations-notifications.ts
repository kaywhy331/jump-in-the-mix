...
export async function postOperationsNotice(destination: URL, notice: { id: string; code: OperationsCode | "monitor"; kind: string; state: string; observedAt: string; evidence: Record<string, number | null> }) {
  try {
  ...
  try {
    const response = await fetch(destination, { method: "POST", redirect: "error", headers: { "Content-Type": "application/json", "Idempotency-Key": notice.id }, body: JSON.stringify({ service: "jump-in-the-mix", eventId: notice.id, check: notice.code, kind: notice.kind, state: notice.state, observedAt: notice.observedAt, evidence: notice.evidence, text }), signal: AbortSignal.timeout(5000) });
