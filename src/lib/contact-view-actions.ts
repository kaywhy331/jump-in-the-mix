...
export async function saveContactViewAction(formData: FormData): Promise<void> {
  ...
  const owner = value(formData, "owner", 120);
  const permission = value(formData, "permission", 40);
  if (!name) fail("Give the saved view a name.");
  const query = { q: q || null, group: group || null, priority: priority || null, owner: owner || null, permission: permission || null } satisfies Prisma.InputJsonObject;
