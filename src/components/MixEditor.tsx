  ...
  return <form ref={formRef} action={saveMixAction} className="mix-editor-form simple-plan-editor" onSubmit={submit}>
    ...
    <section className="card mix-editor-section">
      ...
      <details className="plan-advanced-settings"><summary>Advanced plan settings</summary><div className="form-grid">
        ...
        <label className="field"><span>Goal</span><select name="category" defaultValue={planGoalLabel(mix?.category ?? "")}><option value="">None</option>{[...new Set(categories.map(planGoalLabel))].map((category) => <option key={category}>{category}</option>)}</select></label>
    ...
    <section className="card mix-editor-section">
      <div className="card-header"><div><h2>Follow-ups</h2><p>Add a text, call, or email and choose how many days after the start it should happen.</p></div><button className="button primary" type="button" onClick={addStep}>Add follow-up</button></div>
      <div className={styles.sequenceList}>{sequence.map((item, index) => {
        ...
        return <fieldset className={`mix-editor-jump ${styles.sequenceCard}`} key={item.key}>
          ...
          <div className="form-grid">
            ...
            <label className="field"><span>{signedOffset < 0 ? "Days before start" : "Days after start"}</span><input id={fieldId("day")} type="number" min={0} max={MAX_STEP_OFFSET_DAYS} value={Math.abs(signedOffset)} onChange={(event) => updateStep(index, { dayOffset: (signedOffset < 0 ? -1 : 1) * Number(event.target.value) })} required /></label>
    ...
    <ReviewDialog open={reviewOpen} onClose={() => setReviewOpen(false)} title="Turn on this plan?" description="Check the audience and schedule before starting."><div className={styles.reviewGrid}><div><strong>{audienceEstimate.toLocaleString()}</strong><span>people</span></div><div><strong>{sequence.length}</strong><span>follow-ups each</span></div><div><strong>{projectedFollowUps.toLocaleString()}</strong><span>scheduled follow-ups</span></div></div><div className={styles.reviewWarnings}>{!assignAllContacts && !selectedGroupIds.size && <p className="notice error">Choose who gets this plan.</p>}{channels.includes("EMAIL") && missingEmailCount > 0 && <p>{missingEmailCount} people in your contacts have no email.</p>}{channels.some((channel) => channel !== "EMAIL") && missingPhoneCount > 0 && <p>{missingPhoneCount} people in your contacts have no phone.</p>}</div><div className="form-actions"><button className="button" type="button" onClick={() => setReviewOpen(false)}>Keep editing</button><button className="button primary" type="button" disabled={!assignAllContacts && !selectedGroupIds.size} onClick={confirmActivation}>Turn on plan</button></div></ReviewDialog>
