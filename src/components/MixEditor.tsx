    ...
    <form action={saveMixAction} className="mix-editor-form">
      ...
      <section className="card mix-editor-section">
        ...
        <div className="form-grid">
          ...
          {triggerMode === "DATE_TRIGGERED" && <div className="field"><label htmlFor="mix-date-type">Target Jump Date Type</label><select id="mix-date-type" name="dateTypeId" defaultValue={defaultDateType} required>{dateTypes.map((dateType) => <option key={dateType.id} value={dateType.id}>{dateType.isSystem ? `System · ${dateType.name}` : dateType.name}</option>)}</select></div>}
