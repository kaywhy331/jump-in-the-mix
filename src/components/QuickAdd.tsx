...
export function QuickAddDialog() {
  ...
  return <dialog ref={dialogRef} className="quick-add-dialog" aria-labelledby="quick-add-title" onCancel={(event) => { if (capture.trim()) { event.preventDefault(); closeDialog(); } }}>
    <div className="quick-add-dialog-header"><h2 id="quick-add-title">Quick Add</h2><button className="icon-button" type="button" onClick={closeDialog} aria-label="Close Quick Add"><AppIcon name="close" /></button></div>
    <form className="quick-add-composer" onSubmit={(event) => { event.preventDefault(); if (capture.trim()) setPreview(inferredCapture(capture)); }}>
      ...
      <button className="button primary quick-add-review" type="submit" disabled={!capture.trim()}>Review</button>
    </form>
