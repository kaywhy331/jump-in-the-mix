  ...
  return (
    <form action={useSharedMixTemplateAction} className="mix-editor-form template-setup-form" onSubmit={(event) => {
      ...
      <section className="card mix-editor-section">
        <div className="card-header"><div><h2>Make it yours</h2><p>Start with a draft. You can adjust every message and date.</p></div><span className="status-pill">{stepCount} follow-ups · {durationDays} days</span></div>
        <div className="form-grid">
          <div className="field full"><label htmlFor="template-mix-name">Plan name</label><input id="template-mix-name" name="name" value={name} onChange={(event) => setName(event.target.value)} maxLength={160} required /></div>
      ...
      <section className="card ai-publish-review">
        ...
        <p>Create a draft to review the messages first. Active plans follow your automatic-sending settings, if enabled.</p>
        <div className="sticky-form-actions"><Link className="button" href="/templates">Back to ready-made plans</Link><button className="button primary" type="submit" disabled={!hasAudience}>{status === "ACTIVE" ? "Review and turn on" : "Create plan"}</button></div>
      </section>
      <ReviewDialog open={reviewOpen} onClose={() => setReviewOpen(false)} title={`Turn on ${name || "this plan"}?`} description="Check your audience and sending preferences before starting.">
        <p>{assignAll ? "Everyone" : selected.map(group => group.name).join(", ")} will receive this plan: {stepCount} prepared follow-ups over {durationDays} days.</p>
        <p>{triggerMode === "DATE_TRIGGERED" ? `Follow-ups are scheduled around each person’s ${dateTypeName || "chosen date"}.` : triggerMode === "BROADCAST" ? "The plan starts on the date and time you selected." : "Day 0 is today. The first follow-ups may appear in Today immediately."}</p>
        ...
        <div className="form-actions"><button className="button" type="button" onClick={() => setReviewOpen(false)}>Keep editing</button><button className="button primary" type="submit" data-activation="confirm">Confirm and turn on</button></div>
      </ReviewDialog>
    </form>
  );
}
