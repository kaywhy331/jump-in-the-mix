...
export default async function MorePage() {
  ...
  return <div className="page more-page"><header className="page-header"><div><h1>More</h1><p>Workspace tools, account controls, and help.</p></div></header><div className="settings-hub-grid"><Link className="settings-hub-card desktop-only" href="/jumps"><span className="settings-hub-icon">→</span><span><strong>Today</strong><small>Your queue, upcoming moments, and completed actions.</small></span></Link><Link className="settings-hub-card desktop-only" href="/templates"><span className="settings-hub-icon">→</span><span><strong>Mix Templates</strong><small>Start from a reviewed follow-up plan.</small></span></Link>{destinations.map(([href, title, description], index) => <div className="more-destination" key={href}><span className="more-section-label">{index === 0 ? "Workspace" : index === 1 ? "Account" : "Support"}</span><Link className="settings-hub-card" href={href}><span className="settings-hub-icon desktop-only">→</span><span><strong>{title}</strong><small>{description}</small></span><span className="settings-row-chevron mobile-only" aria-hidden="true">›</span></Link></div>)}<Link className="settings-hub-card" href="/account?section=referrals"><span className="settings-hub-icon desktop-only">→</span><span><strong>Referrals</strong><small>Share Jump in the Mix and review rewards.</small></span><span className="settings-row-chevron mobile-only" aria-hidden="true">›</span></Link>{session.authUser.isPlatformAdmin && <Link className="settings-hub-card" href="/admin"><span className="settings-hub-icon desktop-only">→</span><span><strong>Admin</strong><small>Platform operations and support controls.</small></span><span className="settings-row-chevron mobile-only" aria-hidden="true">›</span></Link>}</div><form action={logoutAction} className="more-sign-out"><button className="button" type="submit">Sign out</button></form></div>;
}
