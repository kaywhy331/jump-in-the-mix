  ...
  return (
    <div className="page admin-support-page">
      <header className="page-header">
        <div>
          <h1>Admin · Support</h1>
          <p>Triage private customer conversations, respond with account context, and inspect email delivery without exposing secrets.</p>
        </div>
      ...
      <section className="stats-grid admin-support-stats">
        ...
        <article className="stat-card"><small>Failed reply emails</small><strong>{failedEmailCount}</strong></article>
      </section>
      ...
      <section className="support-admin-ticket-list">
        {tickets.map((ticket) => {
          ...
          return (
            <Link className={`card support-admin-ticket priority-${ticket.priority.toLowerCase()}`} href={`/admin/support/${ticket.id}`} key={ticket.id}>
