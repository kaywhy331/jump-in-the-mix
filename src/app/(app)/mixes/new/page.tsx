...
export const metadata: Metadata = { title: "Create a mix" };
...
export default async function NewMixPage({ searchParams }: { searchParams: Promise<{ error?: string; custom?: string }> }) {
  ...
  if (query.custom !== "1" && !query.error) {
    ...
    const recommended = !workspace.profile?.industry
      ? RELATIONSHIP_MIXES.slice(0, 6)
      : [...READY_MADE_PLANS.filter((plan) => plan.industry === businessType || plan.industry === "Other").slice(0, 3), ...RELATIONSHIP_MIXES.slice(0, 3)];
    return <div className="page new-plan-choice"><header className="page-header"><div><h1>Create a mix</h1><p>Start with a ready-made mix or build a simple one of your own.</p></div></header><section className="card"><div className="card-header"><div><h2>Find a starting rhythm</h2><p>Choose one and decide who it applies to.</p></div><Link className="button" href="/templates">See every ready-made mix</Link></div><div className="mix-template-grid">{recommended.map((plan) => <article className="mix-template-card" key={plan.id}><span className="status-pill">{plan.category}</span><h3>{plan.title}</h3><p>{plan.description}</p><div className="plan-step-pills">{plan.steps.map((step) => <span key={`${step.name}-${step.dayOffset}`}>{step.channel === "SMS" ? "Text" : step.channel === "PHONE_CALL" ? "Call" : "Email"} · day {step.dayOffset}</span>)}</div><Link className="button primary" href={`/templates/${plan.id}/use`}>Remix it</Link></article>)}</div></section><section className="card"><h2>Build your own</h2><p>Choose who, what channel, what message, and how many days later. Advanced settings stay out of the way.</p><Link className="button" href="/mixes/new?custom=1">Create a mix</Link></section></div>;
  }
  ...
  return (
    <div className="page">
      <header className="page-header"><div><h1>Create a mix</h1><p>Every relationship starts with a first beat. Add messages and reminders, then set the tempo.</p></div></header>
