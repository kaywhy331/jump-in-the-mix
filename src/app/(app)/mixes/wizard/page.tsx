    ...
    <div className="page ai-wizard-page">
      ...
      <form action={generateAiMixDraftAction} className="ai-wizard-form">
        ...
        <fieldset className="card ai-wizard-section">
          ...
          <div className="form-grid">
            <div className="field"><label htmlFor="triggerMode">How should the Mix start?</label><select id="triggerMode" name="triggerMode" defaultValue="MANUAL_START"><option value="MANUAL_START">Start manually</option><option value="DATE_TRIGGERED">From an Important Date</option><option value="BROADCAST">On one fixed date</option></select></div>
            <div className="field"><label htmlFor="dateTypeId">Important Date Type</label><select id="dateTypeId" name="dateTypeId" defaultValue=""><option value="">Used only when starting from an Important Date</option>{dateTypes.map((item) => <option key={item.id} value={item.id}>{item.isSystem ? "System" : "Custom"} · {item.name}</option>)}</select></div>
