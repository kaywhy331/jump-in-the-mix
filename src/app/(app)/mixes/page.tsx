...
export default async function MixesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="page">
      ...
      <header className="page-header"><div><h1>Mixes</h1><p>Follow-up plans that turn an Important Date or assignment into a clear action sequence.</p></div><div className="page-actions"><Link className="button mobile-header-action" href="/templates"><AppIcon name="community" /> Templates</Link>{PLAN_LIMITS[workspace.planTier].aiWizard && <Link className="button mobile-header-action" href="/mixes/wizard"><AppIcon name="bolt" /> AI review</Link>}<Link className="button primary mobile-header-action" href="/mixes/new"><AppIcon name="add" /> Create plan</Link></div></header>
      ...
      <form className="filter-bar" action="/mixes" method="get"><input name="q" defaultValue={q} placeholder="Search Mixes" aria-label="Search Mixes"/><select name="status" defaultValue={status} aria-label="Filter Mixes by status"><option value="">Current Mixes</option><option value="DRAFT">Drafts</option><option value="ACTIVE">Active</option><option value="PAUSED">Paused</option><option value="ARCHIVED">Archived</option></select><button className="button" type="submit">Apply</button>{(q || status) && <Link className="button" href="/mixes">Clear</Link>}</form>
