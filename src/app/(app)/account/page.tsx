...
export default async function AccountPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <div className="page account-page">
    ...
    {section === "privacy" && <><section className="card"><div className="card-header"><div><h2>Personal data</h2><p>Download a copy you can open in a spreadsheet, or a complete JSON backup.</p></div></div><div className="page-actions"><a className="button primary" href="/api/account/export?format=csv">Spreadsheet bundle</a><a className="button" href="/api/account/export">Complete JSON</a></div></section><section className="card danger-zone" aria-labelledby="danger-zone-heading"><div className="card-header"><div><h2 id="danger-zone-heading">Permanently delete account</h2><p>This cannot be undone. Reauthentication and an exact confirmation phrase are required.</p></div></div><div className="notice error"><p><strong>Deletion removes:</strong> your profile, sessions, contacts and private notes, saved dates, tags, mixes, follow-ups, and support history.</p><p>A pseudonymous operational record retains only the deletion time and outcome; it does not retain your name, email, relationship data, or messages.</p></div><AccountDeletionForm hasPassword={hasPassword}/></section></>}
  </div>;
}
