...
export default async function AccountPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (params.section === "support") redirect("/account/tickets");
  ...
  const emailStatus = user.emailVerifiedAt
    ? `Verified ${formatDate(user.emailVerifiedAt, displayPreferences)}`
  ...
  return <div className="page account-page">
    <header className="page-header"><div><h1>My Account</h1><p>Your profile, security, and personal data.</p></div></header>
    <nav className="account-section-nav" aria-label="Account sections">{[["overview", "Profile"], ["security", "Security"], ["privacy", "Data & privacy"]].map(([key, label]) => <Link aria-current={section === key ? "page" : undefined} className={section === key ? "active" : ""} href={`/account?section=${key}`} key={key}>{label}</Link>)}</nav>
    ...
    {section === "overview" && <div className="account-grid"><section className="card account-summary-card"><div className="card-header"><div><h2>Profile</h2><p>Your personal account details and defaults.</p></div><Link className="button small" href="/account/preferences">Edit preferences</Link></div><dl className="account-definition-list"><div><dt>Name</dt><dd>{user.name}</dd></div><div><dt>Email</dt><dd>{user.email}</dd></div><div><dt>Email status</dt><dd>{emailStatus}</dd></div></dl></section><section className="card"><div className="card-header"><div><h2>Your data, your choice</h2><p>Download your information or manage your account in Data &amp; privacy.</p></div></div><Link className="button" href="/account?section=privacy">Manage personal data</Link></section></div>}

    {section === "security" && <><section className="card account-password-card"><div className="card-header"><div><h2>{hasPassword ? "Change password" : "Set a password"}</h2><p>{hasPassword ? "Changing it keeps this device signed in and closes every other session." : "Add a password as a backup to Google, Apple, or email-link sign-in."}</p></div></div><form action={changePasswordAction} className="form-stack">{hasPassword && <div className="field"><label htmlFor="currentPassword">Current password</label><input id="currentPassword" name="currentPassword" type="password" autoComplete="current-password" required /></div>}<div className="field"><label htmlFor="password">New password</label><input id="password" name="password" type="password" autoComplete="new-password" minLength={12} maxLength={72} required /><small>Use at least 12 characters.</small></div><div className="field"><label htmlFor="confirmPassword">Confirm new password</label><input id="confirmPassword" name="confirmPassword" type="password" autoComplete="new-password" minLength={12} maxLength={72} required /></div><button className="button primary" type="submit">{hasPassword ? "Update password" : "Set password"}</button></form></section><section className="card account-sessions-card"><div className="card-header"><div><h2>Active sessions</h2><p>Review devices that currently have access to your account.</p></div>{sessions.length > 1 && <form action={signOutOtherSessionsAction}><button className="button" type="submit">Sign out other devices</button></form>}</div><div className="session-list">{sessions.map((item) => { const current = item.id === session.id; return <article className="session-row" key={item.id}><div className="session-device-icon"><AppIcon name={current ? "check" : "circle"}/></div><div><h3>{describeUserAgent(item.userAgent)} {current && <span className="status-pill done">Current</span>}</h3><p>{item.ipAddress || "IP unavailable"} · Last active {formatDateTime(item.lastSeenAt, displayPreferences)}</p><small>Created {formatDateTime(item.createdAt, displayPreferences)} · Expires {formatDate(item.expiresAt, displayPreferences)}</small></div>{!current && <form action={revokeSessionAction}><input type="hidden" name="sessionId" value={item.id}/><button className="button small danger" type="submit">Sign out</button></form>}</article>; })}</div><div className="account-signout-all"><ConfirmDialog trigger="Sign out everywhere…" title="Sign out everywhere?" description="This closes every active session, including this device." danger><form action={signOutEverywhereAction}><button className="button danger" type="submit">Confirm sign out everywhere</button></form></ConfirmDialog></div></section></>}

    {section === "privacy" && <><section className="card"><div className="card-header"><div><h2>Personal data</h2><p>Download a copy you can open in a spreadsheet, or a complete JSON backup.</p></div></div><div className="page-actions"><a className="button primary" href="/api/account/export?format=csv">Spreadsheet bundle</a><a className="button" href="/api/account/export">Complete JSON</a></div></section><section className="card danger-zone" aria-labelledby="danger-zone-heading"><div className="card-header"><div><p className="eyebrow">Danger Zone</p><h2 id="danger-zone-heading">Permanently delete account</h2><p>This cannot be undone. Reauthentication and an exact confirmation phrase are required.</p></div></div><div className="notice error"><p><strong>Deletion removes:</strong> your profile, sessions, contacts and private notes, saved dates, tags, mixes, follow-ups, and support history.</p><p>A pseudonymous operational record retains only the deletion time and outcome; it does not retain your name, email, relationship data, or messages.</p></div><AccountDeletionForm hasPassword={hasPassword}/></section></>}
  </div>;
}
