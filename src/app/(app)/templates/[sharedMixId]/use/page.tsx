...
export default async function UseMixTemplatePage({ params, searchParams }: { params: Promise<{ sharedMixId: string }>; searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="page template-use-page">
      ...
      <header className="page-header"><div><h1>Set up your plan</h1><p>{shared.title}</p></div><Link className="button" href="/templates">Plan library</Link></header>
      <div className="template-use-grid">
        ...
        <aside className="card template-use-reference"><p className="library-goal">Your starting point</p><h2>{shared.title}</h2><p>{shared.description}</p><p className="library-plan-facts"><span>{steps.length} follow-ups</span><span>{durationDays} days</span></p><details className="plan-message-reference"><summary>Review prepared messages</summary><SharedMixPreview title={shared.title} triggerMode={metadata?.triggerMode ?? "MANUAL_START"} dateTypeName={metadata?.dateTypeName ?? null} durationDays={durationDays} steps={steps} expanded /></details>{salesPlan && <PlanApproachGuide plan={salesPlan}/>}<p className="muted-copy">When someone replies, pause their plan and continue the conversation. Replies are not detected automatically.</p></aside>
      </div>
    </div>
  );
}
