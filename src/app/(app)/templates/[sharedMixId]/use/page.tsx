...
export default async function UseMixTemplatePage({
  params,
  searchParams
}: {
  ...
  const [shared, metadata, rawGroups, groupStates, activeContactCount, activeMixCount] = await Promise.all([
  ...
  return (
    <div className="page template-use-page">
      ...
      <div className="template-use-grid">
        <section className="card template-use-preview"><SharedMixPreview title={shared.title} description={shared.description} triggerMode={metadata?.triggerMode ?? "MANUAL_START"} dateTypeName={metadata?.dateTypeName ?? null} durationDays={shared.durationDays} steps={steps} /></section>
        <TemplateUseForm
          sharedMixId={shared.id}
          requestId={`use-${randomUUID()}`}
          title={shared.title}
          ...
          groups={groups.map((group) => ({ id: group.id, name: group.name, color: group.color, contactCount: group._count.memberships }))}
