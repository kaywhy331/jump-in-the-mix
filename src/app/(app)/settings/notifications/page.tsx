...
export default async function NotificationsPage({ searchParams }: { searchParams: Promise<{ saved?: string; automationSaved?: string; error?: string }> }) {
  ...
  return <div className="page">
    ...
    <section className="card"><div className="card-header"><div><h2>Email reminders</h2><p>Choose the email reminders you want. Sent to {user.email} in your saved timezone.</p></div></div>{!emailConfigured && <Notice type="info">Email reminders are not available for this account yet. You can use push reminders on your device instead.</Notice>}{emailConfigured && <form action={updateNotificationPreferencesAction} className="form-stack"><label className="checkbox-card"><input type="checkbox" name="emailDigestEnabled" defaultChecked={preference?.emailDigestEnabled ?? false} disabled={!emailConfigured} /><span><strong>Morning digest</strong><small>A short list of due and overdue follow-ups.</small></span></label><label className="checkbox-card"><input type="checkbox" name="weeklyReportEnabled" defaultChecked={preference?.weeklyReportEnabled ?? false} disabled={!emailConfigured} /><span><strong>Monday owner report</strong><small>Completed follow-ups, quiet customers, and referrals.</small></span></label><label className="field"><span>Send at</span><select name="digestHour" defaultValue={String(preference?.digestHour ?? 7)} disabled={!emailConfigured}>{Array.from({ length: 24 }, (_, hour) => <option value={hour} key={hour}>{new Intl.DateTimeFormat("en-US", { hour: "numeric", timeZone: "UTC" }).format(new Date(Date.UTC(2020, 0, 1, hour)))}</option>)}</select></label><button className="button primary" type="submit" disabled={!emailConfigured}>Save email reminders</button></form>}</section>
